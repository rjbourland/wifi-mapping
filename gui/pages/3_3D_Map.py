"""3D Map — Point cloud and occupancy grid visualization."""
...
st.markdown(
    ...
    "Point cloud accumulation • Occupancy grid • 3D visualization"
    "</span>",
...
with col_ctrl2:
    ...
    if st.button("🗑 Clear Point Cloud", use_container_width=True):
        st.session_state.point_cloud = __import__("src.mapping.point_cloud", fromlist=["PointCloudAccumulator"]).PointCloudAccumulator(voxel_size)
        st.rerun()
...
# --- Statistics ---
section_header("Point Cloud Stats", "📊")
...
# --- Occupancy Grid (if initialized) ---
section_header("Occupancy Grid", "🧱")

slice_height = st.slider("Z-Slice Height (m)", 0.0, room["height_z"], 1.0, 0.1, key="map_zslice")
st.info("Occupancy grid visualization will be available after collecting localization data with ray-casting enabled.")
